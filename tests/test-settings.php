...
class Test_Settings extends WP_UnitTestCase {
	...
	function test_ds_npr_api_query_publish_callback() {
		...
		ds_npr_api_query_publish_callback($i);
	}

	function test_ds_npr_api_query_callback() {
		...
		ds_npr_api_query_callback($i);
	}

	function test_ds_npr_api_num_multi_callback() {
		update_option( 'ds_npr_num', 'test' );
		$this->expectOutputRegex('/<input type\=\'text\' value=\'test\'.*/');
		ds_npr_api_num_multi_callback();
	}

	function test_ds_npr_api_key_callback() {
		update_option( 'ds_npr_api_key', 'test' );
		$this->expectOutputRegex('/<input type\=\'text\' value=\'test\'.*/');
		ds_npr_api_key_callback();
	}

	function test_ds_npr_api_pull_url_callback() {
		update_option( 'ds_npr_api_pull_url', 'test' );
		$this->expectOutputRegex('/<input type\=\'text\' value=\'test\'.*/');
		ds_npr_api_pull_url_callback();
	}

	function test_ds_npr_api_push_url_callback() {
		update_option( 'ds_npr_api_push_url', 'test' );
		$this->expectOutputRegex('/<input type\=\'text\' value=\'test\'.*/');
		ds_npr_api_push_url_callback();
	}

	function test_ds_npr_api_org_id_callback() {
		update_option( 'ds_npr_api_org_id', 'test' );
		$this->expectOutputRegex('/<input type\=\'text\' value=\'test\'.*/');
		ds_npr_api_org_id_callback();
	}

	function test_ds_npr_pull_post_type_callback() {
		$field_name = 'ds_npr_pull_post_type';
		$this->expectOutputRegex('/<div><select id\=' . $field_name . '.*/');
		ds_npr_pull_post_type_callback();
	}

	function test_ds_npr_push_post_type_callback() {
		$field_name = 'ds_npr_push_post_type';
		$this->expectOutputRegex('/<div><select id\=' . $field_name . '.*/');
		ds_npr_push_post_type_callback();
	}

	function test_ds_npr_push_story_permissions_callback() {
		...
		ds_npr_push_story_permissions_callback();
	}
